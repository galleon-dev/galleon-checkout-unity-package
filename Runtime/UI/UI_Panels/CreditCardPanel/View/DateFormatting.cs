...
{
    /// <summary>Class to format text as date (00/00)</summary>
    public class DateFormatting : LiveDecorationFilter
    {
        public AdvancedInputField DateAdvancedInputField;
        /// <summary>The maximum amount of number characters</summary>
        private const int MAX_NUMBERS = 4;
        ...
        public override string ProcessText(string text, int caretPosition)
        {
            ...
            {
                DateAdvancedInputField.Text = DateAdvancedInputField.Text.Remove(length - 1);
            ...
            else
            {
                for (int i = 0; i < length; i++)
                {
                    ...
                    if (IsNumber(c))
                    {
                        ...
                        if (numberCount == 3) //3rd number found - add separator char
                        {
