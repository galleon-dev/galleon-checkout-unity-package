...
namespace Galleon.Checkout.ELEMENTS
{
    ...
    public class ThingElement : Element
    {
        ...
        =>
            new Step(name   : $"create_thing_asset"
                    ,action : async (s) =>
                    {
                        ...
                        var path = $"{FolderPath}{ve.thingData.ThingName}";
        ...
        =>
            new Step(name   : $"create_thing_hierarchy"
                    ,action : async (s) =>
                    {
                        ...
                        // Get paths
                        string prefabPath   = System.IO.Path.Combine($"Assets/package1/{ve.thingData.ThingName}/Prefabs",   ve.thingData.ThingName + ".prefab");
                        string materialPath = System.IO.Path.Combine($"Assets/package1/{ve.thingData.ThingName}/Materials", ve.thingData.ThingName + "_material.mat");
        ...
        public Step TempCreateThing(VirtualEntity ve) 
        =>
            new Step(name   : $"TempCreateThing"
                    ,action : async (s) =>
                    {
                        // Create parent GameObject with the thing name
                        GameObject parentGO = new GameObject(ve.thingData.ThingName);

                        // Apply position from tags if available
                        Vector3? position = GetPositionFromTags(ve.Tags);
                        if (position.HasValue)
                            parentGO.transform.position = position.Value;
...
";
            ...
            string scriptContent = scriptTemplate.Replace("CLASS_NAME", ve.thingData.ThingName);

            // Write the script file
            string scriptPath = System.IO.Path.Combine(scriptFolder, ve.thingData.ThingName + ".cs");
